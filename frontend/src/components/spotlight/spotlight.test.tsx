...
describe('Spotlight', () => {
  it('renderiza Mortgage30-like com period default 1A', () => {
    ...
    const html = renderToStaticMarkup(<Spotlight indicator={ind} />);
  ...
  it('respeita initialPeriod=1M (slice menor da série)', () => {
    ...
    const html = renderToStaticMarkup(<Spotlight indicator={ind} initialPeriod="1M" />);
  ...
  it('respeita initialPeriod=5A (série completa)', () => {
    ...
    const html = renderToStaticMarkup(<Spotlight indicator={ind} initialPeriod="5A" />);
    expect(html).toMatchSnapshot();
  });
